...
class AnimalSoundsModeTab(QWidget):
    sliderValueChanged = pyqtSignal(int, int)

    def __init__(self):
        super().__init__()

        self.layout = QVBoxLayout()
        self.animal_slider_values = {}
    ...
    def create_sliders(self):
        ...
        for i in range(num_sliders):
            ...
            slider_label = QLabel(f"{animal_names[i]} Slider")  # Use the animal name here
            ...
            self.animal_slider_values[i] = 50
    ...
    def update_slider_value(self, index, value, label):
        ...
        self.animal_slider_values[index] = value
        label.setText(str(value))
        print(f"Animal Slider {index + 1} Value: {value}")
        self.sliderValueChanged.emit(index, value)
